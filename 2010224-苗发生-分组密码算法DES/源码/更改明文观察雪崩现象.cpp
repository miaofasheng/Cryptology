...
int main() {
	...
	result1 = Encryption(plaintext);

	int endl1 = 0;
	for (int i = 0; i < 64; i++)
	{
		plaintext.flip(i);
		result2 = Encryption(plaintext);
	    for (int j = 0; j < 64; j++)
		{
	...
	cout << "*************************************" << endl;
	cout << "单独修改明文的每一个bit，密文平均更改位数为：" << num / 64<<endl;
