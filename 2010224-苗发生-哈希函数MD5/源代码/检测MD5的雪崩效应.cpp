...
int Compaer_Bit_Count(string NewSumry, string OrigiSumry)
{
	int Difference = 0;
	for (int part = 0; part < 8; part++)
	{
		...
		long NewMD5_Int = static_cast<long>(strtol(NewSumry_temp.c_str(), &END_one, 16));
		string OrigiSumry_temp = OrigiSumry.substr(0 + 4 * part, 4);
		long OrigiMD5_Int = static_cast<long>(strtol(OrigiSumry_temp.c_str(), &End_two, 16));
		for (int round = 0; round < 16; round++)
		{
			if (NewMD5_Int % 2 != OrigiMD5_Int % 2)
			{
				Difference++;
			}
			NewMD5_Int /= 2;
...
int main() {
	string Initialtxt = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvw"; 
	string InitialMD5; 
	string NewChangetxt[50]; 
	string NewChangeMD5[50]; 
	int MD5Change[50]; 
	double Average = 0.0;
	cout << "原始消息：" << Initialtxt << endl; 
	InitialMD5 = MD5(Initialtxt).getstring();
	cout << "原始MD5："  << endl;
	Print(Initialtxt);
	for (int i = 0; i < Times; i++) {
		cout << "进行第" << i + 1 << "次雪崩测试" << endl;
		cout << "原始消息："<< Initialtxt <<endl;
		NewChangetxt[i] = Initialtxt;
		if (NewChangetxt[i][i] % 2 == 1)
			NewChangetxt[i][i] -= 1;
		else
			NewChangetxt[i][i] += 1;
		cout << "新消息" << i + 1 << "：" << NewChangetxt[i] << endl;
		cout << "原始MD5：" << endl;
		Print(Initialtxt);
		NewChangeMD5[i] = MD5(NewChangetxt[i]).getstring();
		cout << "新MD5" << i + 1 << "："<<endl;
		Print(NewChangetxt[i]);
		MD5Change[i] = Compaer_Bit_Count(NewChangeMD5[i], InitialMD5);
		cout << "MD5相差二进制位数：" << MD5Change[i] << endl; cout << endl << endl;
	}
	cout << "MD5值位数改变情况：" << endl;
	for (int t = 0; t < Times; t++) 
	{
		Average += MD5Change[t]; 
		cout << MD5Change[t] << " "; 
		if (t == Times/2-1) cout << endl;
	}
	cout << endl; Average /= Times; 
	cout << "MD5值平均改变位数为：" << Average << "位" << endl; system("pause"); return 0;
}